...
# Height of the human in the real world (in meters)
human_height = 1.7
...
def detect_humans_estimate_distance_and_track_motion(frame):
    ...
    for i in nms_indices:
        ...
        if previous_center_x is not None:
            if abs(previous_center_x - center_x) > 1.0:
                direction_x = "Moving Left" if center_x < previous_center_x else "Moving Right"
        previous_center_x = center_x

        # Track vertical motion and determine direction (up or down)
        if previous_center_y is not None:
            if abs(previous_center_y - center_y) > 1.0:
                direction_y = "Moving Front" if center_y < previous_center_y else "Moving Back"
